...
- **Track in Linear** — a second checkbox beside it, with Linear's mark (on by default, remembered; Claude Code only, since it is the CLI with the Linear connector). The clone does nothing in Linear until it is about to change a file for the first time — a question, a read or a plan never raises a ticket. At that moment it picks up the issue you named in the optional field (`LKG-42` or a linear.app URL), or searches the team for an open issue that already describes the work, or creates one: assigned to you, In Progress, with the goal in the description. From then on the clone manages it — the key goes into its branch, commits and PR (`Fixes LKG-42`, so Linear links them and closes on merge), the issue moves to In Review when the PR opens, and on shipping it gets a comment saying what changed, what was verified and what remains, with follow-ups as sub-issues. Kamino reads the issue out of the clone's own `save_issue` result, so a `◆ LKG-42` chip appears on the card, pane and HUD, click to open. Same delivery as the ship orders: system prompt, not a first message, so it holds for the whole session.
