/**
 * Linear's mark — the quarter-circle of stacked bars — for every place a
 * Linear issue is named: the launch-dialog checkbox and the issue chips.
 * Drawn in currentColor so it takes the chip's tone.
 */
...
export function LinearMark(props: { className?: string; title?: string }): React.JSX.Element {
  return (
    <svg
      viewBox="0 0 100 100"
      className={`linear-mark${props.className ? ` ${props.className}` : ''}`}
